...
% Get ergodic averages of various quantities
    ...
    if Nd == length(sol.eta) - 1 % Then no Infs appeared during ergocalc
        ...
        out.household_premium = ergodic_mean(sol.household_premium(i), sol.density, sol.eta_density);
        out.solvency_premium = ergodic_mean(sol.solvency_premium(i), sol.density, sol.eta_density);
        ...
        out.household_premium = ergodic_mean(sol.household_premium(ia), sol.density(ib), eta);
        out.solvency_premium = ergodic_mean(sol.solvency_premium(ia), sol.density(ib), eta);
