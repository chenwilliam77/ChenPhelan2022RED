...
% fd_type: 1 for central difference, 2 for finite difference
...
function out = get_fd_coef(deriv, acc, x0, gridpt, fd_type, all)
    ...
    elseif fd_type == 2
        % one-sided
        adjust = acc + (deriv - 1); % one more point per accuracy order, one more for each derivative above 1
        if i == 1
            % forward
            grid_samp = gridpt(i:i + adjust);
        elseif i == length(gridpt)
            % backward
            grid_samp = gridpt(i-adjust:i);
        end
    end
    out = fornberg(deriv, x0, grid_samp, all);
end

function out = fornberg(deriv, x0, gridpt, all)
