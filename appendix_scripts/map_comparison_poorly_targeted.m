...
crisis_constraints_1 = [Inf; 35; 30];
etagrid_pt = .02;
crisis_constraints_2 = [Inf; 25; 25]; % second node of crisis constraints
N_mp_rules = 5;
...
lc_linear_calibration_parameters;
s.leverage_constraint.type = 'linear';
s.leverage_constraint.etagrid = [0; etagrid_pt; 1];
s.leverage_constraint.Ls = [crisis_constraints_1(1); crisis_constraints_2(1); Inf];
s.plot_results = 0;
s.use_title = 0;
s.verbose = 0;
s.iput = best_rule(1);
...
s.leverage_constraint.type = 'linear';
s.leverage_constraint.etagrid = [0; etagrid_pt; 1];
s.leverage_constraint.Ls = [crisis_constraints_1(2); crisis_constraints_2(2); Inf];
s.plot_results = 0;
s.use_title = 0;
s.color = 'b';
s.verbose = 0;
...
rule3_stats.stab_frac_pct50 = zeros(N_mp_rules, 1);
...
s.leverage_constraint.type = 'linear';
s.leverage_constraint.etagrid = [0; etagrid_pt; 1];
s.leverage_constraint.Ls = [crisis_constraints_1(3); crisis_constraints_2(3); Inf];
s.plot_results = 0;
s.use_title = 0;
s.iput = best_rule(1);
s.ilaw = best_rule(2);
...
% OptNoMaP
rule3_stats.welfare(1) = 0;
rule3_stats.stab_frac(1) = 0;
rule3_stats.stab_frac_pct50(1) = 0;
...
% Inflation targeting
[constpi_s, converged] = find_constant_inflation_mp_rule(L3s, 'damping', 0.7, 'tol', 1e-2, 'thinning', 5);
if ~converged
    disp('Inflation targeting rule not found! Figure out what is wrong');
    keyboard;
end
...
fprintf('(ExpV, Pr. Distress, Pr. Crisis) for Rule 3:     (%.2f, %.2f%%, %.2f%%)\n', ...
        [rule3_sumstats{1}.ergodic.ExpV, 100 - rule3_sumstats{3}.ergodic.stab_frac, ...
