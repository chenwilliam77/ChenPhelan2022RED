% This script maximizes expected welfare for a linear Fed Put.
% Thus, there are 4 arguments over which to
% optimize, iput, ilaw, etaPUT, and etaLAW - etaPUT.
% We also set lambda to half its calibrated value.
%
...
savepath = [pwd(), '/../save/output_data/max_expV_fedput_4args_lambda_half/'];
if ~isdir(savepath)
    mkdir(savepath);
end
...
s.verbose = 0;
s.max_expV = 1;
s.lambda = s.lambda / 2; % 1/2 inflation costs
...
ub = [.04; s.imax; .05; .10];
algo = 'interior-point';
Ns = [4, 4, 5, 5]; % dimension of initial guesses w/in hyper cube specified by lb and ub
